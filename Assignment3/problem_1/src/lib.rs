...
pub mod Subprocess { // Module using System Calls.
    ...
    pub fn subprocessing(mut ins_vec: Vec<String>) -> Option<()>{
            ...
            match unsafe { fork() } {
                Ok(ForkResult::Parent {child }) => {
                    match waitpid(child, None){
                        Ok(status) => println!{"[Oh My Shell] Child process terminated: {:?}, status: {:?}",child,status},
                        _ => {
                            println!{"Failed to wait child"};
                            return None;
                        }
                    }   
                }
        ...
        let mut exit_codes = Vec::new(); // vector for storing exit codes

        for child in children {
            if let Ok(status) = waitpid(child, None) {
                match status {
                    WaitStatus::Exited(_, code) => {
                        exit_codes.push((child, Some(code)));
                    },
                    WaitStatus::Signaled(_, _sig, _core_dumped) => {
                        // None for exit by signall
                        exit_codes.push((child, None));
                    },
                    _ => {
                        exit_codes.push((child, None));
                    }
                ...
                // if fail watipid, None.
                exit_codes.push((child, None));
            }
        }

        for (pid, code_opt) in exit_codes {
            match code_opt {
                Some(code) => println!{"[Oh My Shell] Child process terminated: {:?}, status: {:?}",pid,code},
                None => println!("[Oh My Shell] Child {pid} did not exit normally (signaled or error)"),
            }
        }
